// import React, { useEffect, useState } from "react";
// import { FaStar } from "react-icons/fa";
// import { TbClipboardList } from "react-icons/tb";
// import { WiTime5 } from "react-icons/wi";
// import { LuUser2 } from "react-icons/lu";
// import { FiCheckCircle } from "react-icons/fi";
// import { useNavigate } from "react-router-dom";
// import ReactPaginate from "react-paginate";
// import { BsSortDown, BsSortDownAlt } from "react-icons/bs";
// import Button from "../components/Button";
// import { FaEye } from "react-icons/fa";

// function ListDocument() {
//   const [documents, setDocuments] = useState([]);
//   const [currentPage, setCurrentPage] = useState(0);
//   const itemsPerPage = 12;
//   const navigate = useNavigate();
//   const [sortOrder, setSortOrder] = useState("");

//   useEffect(() => {
//     fetchDocuments();
//   }, [sortOrder]);

//   const fetchDocuments = () => {
//     const baseURL = "http://localhost:8080/api/documents";
//     const url =
//       sortOrder === "view"
//         ? `${baseURL}/sorted?sortBy=view&order=asc`
//         : sortOrder === "view_desc"
//         ? `${baseURL}/sorted?sortBy=view&order=desc`
//         : sortOrder === "createdAt"
//         ? `${baseURL}/sorted?sortBy=createdAt&order=desc`
//         : sortOrder === "createdAt_asc"
//         ? `${baseURL}/sorted?sortBy=createdAt&order=asc`
//         : sortOrder === "a_to_z"
//         ? `${baseURL}/sorted?sortBy=view&order=asc` // A to Z sorting
//         : sortOrder === "z_to_a"
//         ? `${baseURL}/sorted?sortBy=view&order=desc` // Z to A sorting
//         : baseURL;

//     fetch(url)
//       .then((response) => {
//         if (!response.ok) {
//           throw new Error("Network response was not ok");
//         }
//         return response.json();
//       })
//       .then((data) => {
//         console.log(data); // Log the response data
//         setDocuments(data);
//       })
//       .catch((error) => console.error("Error fetching documents:", error));
//   };

//   const handleItemClick = (id) => {
//     navigate(`/documents/${id}`);
//   };

//   const handlePageClick = ({ selected }) => {
//     setCurrentPage(selected);
//   };

//   const handleSortByMostViewed = () => {
//     setSortOrder("view_desc");
//     setCurrentPage(0); // Reset to first page when sorting
//   };

//   const handleSortByLeastViewed = () => {
//     setSortOrder("view");
//     setCurrentPage(0); // Reset to first page when sorting
//   };

//   const handleSortByNewest = () => {
//     setSortOrder("createdAt");
//     setCurrentPage(0); // Reset to first page when sorting
//   };

//   const handleSortByOldest = () => {
//     setSortOrder("createdAt_asc");
//     setCurrentPage(0); // Reset to first page when sorting
//   };

//   const handleSortAtoZ = () => {
//     setSortOrder("a_to_z");
//     setCurrentPage(0); // Reset to first page when sorting
//   };

//   const handleSortZtoA = () => {
//     setSortOrder("z_to_a");
//     setCurrentPage(0); // Reset to first page when sorting
//   };

//   const handleShowAllDocuments = () => {
//     setSortOrder(""); // Đặt sortOrder về mặc định để lấy tất cả tài liệu
//     setCurrentPage(0); // Đặt lại trang hiện tại về trang đầu tiên
//   };

//   const offset = currentPage * itemsPerPage;
//   const currentDocuments = documents.slice(offset, offset + itemsPerPage);

//   return (
//     <div className="containerListDocument">
//       <div className="titleListDocument">Tất cả các tài liệu</div>
//       <div className="containerSort">
//         <div className="titleSort">Sắp xếp: </div>
//         <div className="filterContainer">
//           <Button className="buttonSort" onClick={handleShowAllDocuments}>
//             <BsSortDownAlt className="iconSort" />
//             <span className="titleButtonIcon">Tất cả</span>
//           </Button>
//           <Button className="buttonSort" onClick={handleSortByNewest}>
//             <BsSortDownAlt className="iconSort" />
//             <span className="titleButtonIcon">Ngày tạo mới nhất</span>
//           </Button>
//           <Button className="buttonSort" onClick={handleSortByOldest}>
//             <BsSortDown className="iconSort" />
//             <span className="titleButtonIcon">Ngày tạo cũ nhất</span>
//           </Button>
//           <Button className="buttonSort" onClick={handleSortByLeastViewed}>
//             <FaEye className="iconSort" />
//             <span className="titleButtonIcon">Xem ít nhất</span>
//           </Button>
//           <Button className="buttonSort" onClick={handleSortByMostViewed}>
//             <FaEye className="iconSort" />
//             <span className="titleButtonIcon">Xem nhiều nhất</span>
//           </Button>
//           <Button className="buttonSort" onClick={handleSortAtoZ}>
//             <BsSortDown className="iconSort" />
//             <span className="titleButtonIcon">Từ A - Z</span>
//           </Button>
//           <Button className="buttonSort" onClick={handleSortZtoA}>
//             <BsSortDown className="iconSort" />
//             <span className="titleButtonIcon">Từ Z - A</span>
//           </Button>
//         </div>
//       </div>
//       <div className="containerListItemDocument">
//         {currentDocuments.length > 0 ? (
//           currentDocuments.map((doc) => (
//             <div
//               key={doc.id}
//               className="itemDocumentList"
//               onClick={() => handleItemClick(doc.id)}
//             >
//               <img
//                 src={doc.image}
//                 alt={doc.title}
//                 className="imgDocumentList"
//               />
//               <div className="listInfo">
//                 <div className="titleInfo">{doc.title}</div>
//                 <div className="listItemInfo">
//                   <TbClipboardList />
//                   Thể loại: {doc.categoryName}
//                 </div>
//                 <div className="listItemInfo">
//                   <WiTime5 />
//                   Thời gian: {doc.relativeUpdatedAt}
//                 </div>
//                 <div className="listItemInfo">
//                   <LuUser2 />
//                   Người Đăng: {doc.author}
//                 </div>
//                 <div className="listItemInfo">
//                   <FaEye />
//                   Lượt xem: {doc.view}
//                 </div>
//                 <div className="listItemInfoAcp">
//                   <FiCheckCircle />
//                   {doc.status}
//                 </div>
//                 <div className="star-rating">
//                   {[1, 2, 3, 4, 5].map((star) => (
//                     <FaStar
//                       key={star}
//                       className={`star ${star <= 5 ? "filled" : ""}`}
//                     />
//                   ))}
//                 </div>
//               </div>
//             </div>
//           ))
//         ) : (
//           <div>Không có tài liệu nào.</div>
//         )}
//       </div>
//       <ReactPaginate
//         previousLabel={"←"}
//         nextLabel={" →"}
//         breakLabel={"..."}
//         pageCount={Math.ceil(documents.length / itemsPerPage)}
//         marginPagesDisplayed={2}
//         pageRangeDisplayed={5}
//         onPageChange={handlePageClick}
//         containerClassName={"pagination"}
//         activeClassName={"active"}
//       />
//     </div>
//   );
// }

// export default ListDocument;

import React, { useEffect, useState } from "react";
import { FaStar } from "react-icons/fa";
import { TbClipboardList } from "react-icons/tb";
import { WiTime5 } from "react-icons/wi";
import { LuUser2 } from "react-icons/lu";
import { FiCheckCircle } from "react-icons/fi";
import { useNavigate } from "react-router-dom";
import ReactPaginate from "react-paginate";
import { BsSortDown, BsSortDownAlt } from "react-icons/bs";
import Button from "../components/Button";
import { FaEye } from "react-icons/fa";
import "./css/index.css";

function ListDocument() {
  const [documents, setDocuments] = useState([]);
  const [currentPage, setCurrentPage] = useState(0);
  const itemsPerPage = 12;
  const navigate = useNavigate();
  const [sortOrder, setSortOrder] = useState("");
  const [isModalOpen, setIsModalOpen] = useState(false); // State for modal visibility

  useEffect(() => {
    fetchDocuments();
  }, [sortOrder]);

  const fetchDocuments = () => {
    const baseURL = "http://localhost:8080/api/documents";
    const url =
      sortOrder === "view"
        ? `${baseURL}/sorted?sortBy=view&order=asc`
        : sortOrder === "view_desc"
        ? `${baseURL}/sorted?sortBy=view&order=desc`
        : sortOrder === "createdAt"
        ? `${baseURL}/sorted?sortBy=createdAt&order=desc`
        : sortOrder === "createdAt_asc"
        ? `${baseURL}/sorted?sortBy=createdAt&order=asc`
        : sortOrder === "a_to_z"
        ? `${baseURL}/sorted?sortBy=view&order=asc`
        : sortOrder === "z_to_a"
        ? `${baseURL}/sorted?sortBy=view&order=desc`
        : baseURL;

    fetch(url)
      .then((response) => {
        if (!response.ok) {
          throw new Error("Network response was not ok");
        }
        return response.json();
      })
      .then((data) => {
        console.log(data);
        setDocuments(data);
      })
      .catch((error) => console.error("Error fetching documents:", error));
  };

  const handleItemClick = (id) => {
    navigate(`/documents/${id}`);
  };

  const handlePageClick = ({ selected }) => {
    setCurrentPage(selected);
  };

  const handleShowAllDocuments = () => {
    setSortOrder("");
    setCurrentPage(0);
  };

  const offset = currentPage * itemsPerPage;
  const currentDocuments = documents.slice(offset, offset + itemsPerPage);

  const handleSortChange = (order) => {
    setSortOrder(order);
    setCurrentPage(0);
    setIsModalOpen(false); // Close modal after selecting sort
  };

  return (
    <div className="containerListDocument">
      <div className="titleListDocument">Tất cả các tài liệu</div>
      <div className="containerSort">
        <div className="titleSort">Sắp xếp theo</div>
        <div className="filterContainer">
          <Button
            className="buttonSort"
            onClick={() => handleSortChange("view_desc")}
          >
            Xem nhiều nhất
          </Button>
          <Button
            className="buttonSort"
            onClick={() => handleSortChange("view")}
          >
            Xem ít nhất
          </Button>
          <Button className="buttonSort" onClick={() => setIsModalOpen(true)}>
            Xem thêm sắp xếp
          </Button>
        </div>
      </div>
      {isModalOpen && (
        <div className="modalOverlay">
          <div className="sortModal">
            <h2 className="titleSortDocument">Sắp xếp</h2>
            {/* <Button
              className="buttonSort"
              onClick={() => handleSortChange("view_desc")}
            >
              Xem nhiều nhất
            </Button>
            <Button
              className="buttonSort"
              onClick={() => handleSortChange("view")}
            >
              Xem ít nhất
            </Button> */}
            <Button
              className="buttonSort"
              onClick={() => handleSortChange("createdAt")}
            >
              Ngày tạo mới nhất
            </Button>
            <Button
              className="buttonSort"
              onClick={() => handleSortChange("createdAt_asc")}
            >
              Ngày tạo cũ nhất
            </Button>
            <Button
              className="buttonSort"
              onClick={() => handleSortChange("a_to_z")}
            >
              Từ A - Z
            </Button>
            <Button
              className="buttonSort"
              onClick={() => handleSortChange("z_to_a")}
            >
              Từ Z - A
            </Button>
            <Button
              className="buttonSort"
              onClick={() => handleShowAllDocuments()}
            >
              Tất cả
            </Button>
            <Button
              className="buttonClose"
              onClick={() => setIsModalOpen(false)}
            >
              Đóng
            </Button>
          </div>
        </div>
      )}
      <div className="containerListItemDocument">
        {currentDocuments.length > 0 ? (
          currentDocuments.map((doc) => (
            <div
              key={doc.id}
              className="itemDocumentList"
              onClick={() => handleItemClick(doc.id)}
            >
              <img
                src={doc.image}
                alt={doc.title}
                className="imgDocumentList"
              />
              <div className="listInfo">
                <div className="titleInfo">{doc.title}</div>
                <div className="listItemInfo">
                  <TbClipboardList />
                  Thể loại: {doc.categoryName}
                </div>
                <div className="listItemInfo">
                  <WiTime5 />
                  Thời gian: {doc.relativeUpdatedAt}
                </div>
                <div className="listItemInfo">
                  <LuUser2 />
                  Người Đăng: {doc.author}
                </div>
                <div className="listItemInfo">
                  <FaEye />
                  Lượt xem: {doc.view}
                </div>
                <div className="listItemInfoAcp">
                  <FiCheckCircle />
                  {doc.status}
                </div>
                <div className="star-rating">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <FaStar
                      key={star}
                      className={`star ${star <= 5 ? "filled" : ""}`}
                    />
                  ))}
                </div>
              </div>
            </div>
          ))
        ) : (
          <div>Không có tài liệu nào.</div>
        )}
      </div>
      <ReactPaginate
        previousLabel={"←"}
        nextLabel={" →"}
        breakLabel={"..."}
        pageCount={Math.ceil(documents.length / itemsPerPage)}
        marginPagesDisplayed={2}
        pageRangeDisplayed={5}
        onPageChange={handlePageClick}
        containerClassName={"pagination"}
        activeClassName={"active"}
      />
    </div>
  );
}

export default ListDocument;
